...
void findSlimeChunks(const long seed, const int radius, const int runLength,
                     const int centerX = 0, const int centerZ = 0)
{
  // Перевод обычных координат в чанковые
  immutable int chunkCenterX = centerX >> 4;  // / 16
  immutable int chunkCenterZ = centerZ >> 4;  // / 16
  ...
  foreach (ref int z; -radius .. radius + maxHeight)
  {
    ...
    // Fill a row of values — смещаем на чанковый центр
    if (z < radius)
      foreach (x; -radius .. radius)
        arr[(x + radius) + ((z + radius) % maxHeight) * size] =
          isSlimeChunk(seed, x + chunkCenterX, z + chunkCenterZ);
        ...
        if (patternZ + radius + pattern.h < size)
          foreach (x; -radius .. radius - pattern.w)
            if (checkGrid(arr, maxHeight, radius, size, x, patternZ, pattern.w, pattern.h))
              // Возвращаем чанковые координаты относительно центра
              foundPatternCallback(x + chunkCenterX, patternZ + chunkCenterZ,
                                   pattern.w, pattern.h);
    }
